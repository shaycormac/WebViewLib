...
public class NestedWebViewCompat extends WebView implements NestedScrollingChild, 
    ...
    public NestedWebViewCompat(Context context) {
        this(context, null);
    }

    public NestedWebViewCompat(Context context, AttributeSet attrs) {
        this(context, attrs, android.R.attr.webViewStyle);
    }

    public NestedWebViewCompat(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (actionMasked) {
            case MotionEvent.ACTION_DOWN:
                //todo  下面的代码解决一些前端页面一直显示为0的方案
                //最近项目里需要加载WebView，并通过原生的下拉刷新控件控制WebView的刷新。
                // 实现方式很简单就是WebView外面加上下拉刷新控件即可。但是测试时会发现网页无法上拉，
                // 往上滑动就会触发下拉刷新控件的refresh事件。网上查了很多解决办法,有重写WebView的,
                // 有重写下拉刷新控件的(我这里使用的下拉刷新控件是第三方的NestRefreshLayout，其他的刷新控件大同小异)，
                // 但是试过好多以后发现都没有作用。好多方法都是通过webView.getScrollY() 的值来判断是否滚动到顶部，
                // 一次来决定刷新控件是否执行刷新的动作，但是无奈webView.getScrollY() 总是 ＝0，所以只要它小于0，改成1，这样
                //减少与SwipeRefreshLayout的冲突。
                if (getScrollY() <= 0) {
                    scrollTo(0, 1);
                }
