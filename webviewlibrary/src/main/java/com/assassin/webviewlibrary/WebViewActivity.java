...
public class WebViewActivity extends AppCompatActivity 
{
    ...
    private void bindListener() {
        ...
        imgRefresh.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                WebViewSetting.clearWebViewCache(WebViewActivity.this);
    ...
    private void takePhoto() 
    {
        AndPermission.with(this)
                ...
                .onGranted(new Action<List<String>>() 
                {
                    ...
                    {
                        Toast.makeText(WebViewActivity.this, data.toString(), Toast.LENGTH_SHORT)
                                .show();
                      //创建一个空文件
                ...
                .onDenied(new Action<List<String>>() {
                    @Override
                    public void onAction(List<String> data) {
                        Toast.makeText(WebViewActivity.this, data.toString(), Toast.LENGTH_SHORT).show();
                        DialogUtils.alertRefuseDialog(data, WebViewActivity.this, false, false);
                    }
                })
                .start();
    }
    ...
    class MyWebChromeClient extends WebChromeClient {
        ...
        public boolean onJsAlert(WebView view, String url, String message, final JsResult result) {
            //自己加入的弹窗
            DialogUtils.showAlertDialog(WebViewActivity.this, "", message, true, new DialogUtils
                    .AlertDialogListener() {
                @Override
                public void onPositiveSelect()
                {
                    Toast.makeText(WebViewActivity.this, "点击了确定按钮", Toast.LENGTH_SHORT).show();
                }
            });
            result.cancel();
            return true;
                ...
                contentDisposition, String mimetype, long contentLength)
        {
            //首先校验一下读写权限
               final String fileName = DownloadUtil.getNetFileName(contentDisposition, url);
                DialogUtils.showAlertDialog(WebViewActivity.this, "注意", "您要下载"+fileName+"这个文件么", true, new DialogUtils.AlertDialogListener() 
                {
                    ...
                    {
                        //先判断是否有这个文件
                        File tmpFile= DownloadUtil.getDownloadFile(url, WebViewActivity.this);
                        if (tmpFile != null && tmpFile.exists() )
                        {
                            //todo 打开文件
                            DownloadUtil.openPageAccordType(tmpFile,WebViewActivity.this);
                            return;
                        }else 
                        {
                            if (DownloadUtil.isDownloading(url, WebViewActivity.this))
                            {
                                Toast.makeText(WebViewActivity.this, "正在下载~", Toast.LENGTH_SHORT).show();
                                return;
                            }else 
                            {
                                DownloadUtil download = new DownloadUtil(WebViewActivity.this,
                                        url, "文件的名称", fileName);
